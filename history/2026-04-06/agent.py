...
- keywords: product type + brand + material + color + size + use. 3-8 words, include ALL qualifying terms. NEVER start with "shops", "find stores", "look for selling".
...
def _handle_single_product(params: Dict[str, Any], query: str, steps: List[Dict[str, Any]]) -> None:
    ...
    result = _execute_and_record(
        "find_product",
        _build_tool_search_payload(product),
        query,
        steps,
    )
...
def _handle_voucher_search(params: Dict[str, Any], query: str, steps: List[Dict[str, Any]]) -> None:
    """Voucher path: optional same-shop bundle, else independent searches (price filter often off)."""
